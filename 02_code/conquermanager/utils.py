...
first_names = ['Juan', 'Ana', 'Luis', 'María', 'Carlos', 'Sofía', 'Pedro', 'Laura', 'Jorge', 'Elena',
               'Miguel', 'Lucía', 'Pablo', 'Carmen', 'Andrés', 'Paula', 'Roberto', 'Marta', 'Ricardo', 'Eva']
last_names = ['García', 'Martínez', 'Rodríguez', 'López', 'Sánchez', 'González', 'Pérez', 'Hernández', 'Díaz', 'Torres',
...
for family in total_age_per_family:
    print(f'La suma total de las edades en la familia {family.name} es {family.total_age}')
...
results_or = Person.objects.filter(
    Q(first_name='Juan') | Q(age__gt=18)
)
...
results_and = Person.objects.filter(
    Q(first_name='Juan') & Q(age__gt=18)
)
...
for person in persons_with_families:
    print(f'{person.first_name} {person.last_name} pertenece a la familia {person.family.name}')
...
for person in persons_with_families:
    ...
    print(f'{person.first_name} {person.last_name} pertenece a las familias {family_names}')
